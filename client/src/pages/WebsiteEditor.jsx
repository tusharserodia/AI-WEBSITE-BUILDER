...
const WebsiteEditor = () => {
    ...
    const thinkingSteps = ["Understanding your request", "PLanning layout Changes", 
        "Improving responsiveness", "Applying animations", "Finalizing update"
    ]
    ...
    <div className='h-screen w-screen flex bg-black text-white overflow-hidden'>
        ...
        <div className='p-3 border-t border-white/10'>
              <div className='flex gap-2'>
                <input placeholder='Describe Changes..' className='flex-1 resize-none rounded-2xl px-4 py-3 bg-white/5 border border-white/10 
                text-outline-none' onChange={(e) => setPrompt(e.target.value)} value={prompt}>
                    
                </input>
                 <button className='px-4 py-3 rounded-2xl bg-white text-black'
                 onClick={handleUpdate} disabled = {updateloading}>
                        <Send size={14}/>
                    </button>
                </div>            
          </div> 
        ...
        <div className='flex-1 flex flex-col'>
          <div className='h-14 px-4 flex justify-between items-center border-b  border-white/10 bg-black/80'>
             <span className='text-zinc-400 text-xs'>Live preview</span>
             <div className='flex gap-2'>
                {website.deployed ? "" : 
                   <button  onClick = {handleDeploy} className='flex items-center gap-2 px-4 py-1.5 rounded-lg bg-linear-to-r  from-indigo-500  to-purple-500 text-sm font-semmibold hover:scale-105 transition '>
